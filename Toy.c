...
static void toy_mulmv(short* dst, const short* mat, const short* vec)
{
    ...
    {
        for (int i = 0; i < TK_K * TK_N; i += TK_N, idx += TK_N)
            toy_polmul_naive(dst + kv, mat + idx, vec + i, 1);
    }
}

static void toy_mulmvTv(short* dst, const short* mat, const short* vec)
{
    memset(dst, 0, TK_K * TK_N * sizeof(short));
    for (int kv = 0; kv < TK_K * TK_N; kv += TK_N)
    {
        for (int i = 0; i < TK_K * TK_N; i += TK_N)
            toy_polmul_naive(dst + kv, mat + TK_K * i + kv, vec + i, 1);
    }
}
...
static void toy_dot(short* dst, const short* v1, const short* v2)
{
    ...
    for (int i = 0; i < TK_K * TK_N; i += TK_N)
        toy_polmul_naive(dst, v1 + i, v2 + i, 1);
}
...
void toy_enc(const short* A, const short* t, int plain, short* u, short* v)
{
    ...
    for (int i = 0; i < TK_N; ++i)
        v[i] = (v[i] + ((TK_Q >> 1) & -(plain >> (TK_N - 1 - i) & 1))) % TK_Q;
}
